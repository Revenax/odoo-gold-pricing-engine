...
class SilverPriceService(models.Model):
    ...
    def update_all_silver_product_prices(self):
        ...
        Called by cron every 10 minutes. Uses stored price (set by Selenium script).
        """
        _logger.info('Starting silver price update for all products')
        try:
            base_silver = self.get_current_silver_price_999()
            if base_silver <= 0:
                _logger.warning('Silver price not set. Run scripts/selenium_automation.py with ODOO_* env vars.')
                return {
